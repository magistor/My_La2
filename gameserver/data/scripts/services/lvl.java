...
public class lvl extends Functions {
    ...
    public void list() {
        ...
        html = HtmCache.getInstance().getHtml("scripts/services/lvl.htm", player);
        String add = "";
        if (player.getLevel() < Config.SERVICES_LVL_UP_MAX) {
            if (player.isLangRus()) {
                ...
                add += "<button value=\"Raise the level at 1 (Price:" + Config.SERVICES_LVL_UP_PRICE + " " + ItemHolder.getInstance().getTemplate(Config.SERVICES_LVL_UP_ITEM).getName() + ") \" action=\"bypass -h scripts_services.lvl:up" + "\" width=250 height=20 back=\"L2UI_CT1.Button_DF\" fore=\"L2UI_CT1.Button_DF\">" + "</a><br>";
            }
        }
