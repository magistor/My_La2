...
public class ItemBroker extends Functions {
    ...
    public class Item {
        ...
        public Item(int itemId, int type, long price, long count, int enchant, String itemName, long storedId, String merchantName, Location player, int itemObjId, TradeItem item) {
            ...
            this.rare = org.apache.commons.lang3.ArrayUtils.contains(RARE_ITEMS, itemId);
    ...
    public TreeMap<String, TreeMap<Long, Item>> getItems(int type) {
    ...
    public String getHtmlAppends(Integer val) {
        ...
        if (type > 0) {
            if (getSelf().isLangRus()) {
                ...
                if (type != Player.STORE_PRIVATE_MANUFACTURE) {
                    append.append("[scripts_services.ItemBroker:list ").append(type).append(" 90 1 0 0|<font color=\"FF9900\">Стандартные предметы</font>]<br1>"); // отсутствующий элемент
                }
                append.append("<edit var=\"tofind\" width=100><br1>");
                append.append("[scripts_services.ItemBroker:find ").append(type).append(" 1 $tofind|<font color=\"FF9900\">Найти</font>]<br1>"); // DS: разобраться как и почему вместо \\$var стало нужно использовать $var
                ...
                append.append("<edit var=\"tofind\" width=100><br1>");
                append.append("[scripts_services.ItemBroker:find ").append(type).append(" 1 $tofind|<font color=\"FF9900\">Find</font>]<br1>"); // DS: разобраться как и почему вместо \\$var стало нужно использовать $var
    ...
    private void listPageNum(StringBuilder out, int type, int itemType, int page, int minEnchant, int rare, String letter) {
        out.append("<a action=\"bypass -h scripts_services.ItemBroker:list ");
        out.append(type);
        out.append(" ");
        out.append(itemType);
        out.append(" ");
    ...
    public void listForItem(String[] var) {
        ...
        _log.info("type=" + type + " itemId=" + itemId + " minEnchant=" + minEnchant + " rare=" + rare + " itemType=" + itemType + " currentPage=" + currentPage + " returnPage=" + returnPage);
                    ...
                    switch (type) {
                        case Player.STORE_PRIVATE_SELL:
                            ...
                            for (TradeItem item : tradeList) {
                                ...
                                while (key < newItem.price * 100 + 100 && oldItems.containsKey(key)) // До 100 предметов с одинаковыми ценами
                                {
                                    key++;
                                }
