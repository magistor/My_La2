    ...
    protected boolean useItemImpl(Player player, ItemInstance item, boolean ctrl) {
        ...
        switch (itemId) {
            ...
            case 14612:
                ...
                break;
            // Hardin's Divine Protection
            case 21030:
                player.broadcastPacket(new MagicSkillUse(player, player, 22170, 1, 0, 0));
                player.altOnMagicUseTimer(player, SkillTable.getInstance().getInfo(22170, 1));
                break;
            // Hardin's Blessing
            case 21031:
                player.broadcastPacket(new MagicSkillUse(player, player, 22171, 1, 0, 0));
                player.altOnMagicUseTimer(player, SkillTable.getInstance().getInfo(22171, 1));
                break;
            // Silpeed's Wing
            case 21032:
                player.broadcastPacket(new MagicSkillUse(player, player, 22172, 1, 0, 0));
                player.altOnMagicUseTimer(player, SkillTable.getInstance().getInfo(22172, 1));
                break;
            // Silpeed's Blessing
            case 21033:
