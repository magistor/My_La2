...
import l2p.gameserver.model.AggroList.HateComparator;
...
public class OnActionShift extends Functions {
    ...
    public boolean OnActionShift_NpcInstance(Player player, GameObject object) {
        ...
        if (object.isNpc()) {
            ...
            if (Config.ALT_FULL_NPC_STATS_PAGE) {
                ...
                dialog = dialog.replaceFirst("%aspd%", String.valueOf(npc.getPAtkSpd(true)));
    ...
    public void skillsSummon() {
        Player player = getSelf();
        if (player == null) {
            return;
        }
        SummonInstance npc = (SummonInstance) player.getTarget();
        if (npc == null) {
            return;
        }
    ...
    public void stats() {
        ...
        dialog = dialog.replaceFirst("%runSpeed%", String.valueOf(npc.getRunSpeed()));
        dialog = dialog.replaceFirst("%walkSpeed%", String.valueOf(npc.getWalkSpeed()));
        dialog = dialog.replaceFirst("%pAtkSpd%", String.valueOf(npc.getPAtkSpd(true)));
    ...
    public boolean OnActionShift_PetInstance(Player player, GameObject object) {
        ...
        if (object.isPet()) {
            ...
            dialog = dialog.replaceFirst("%pAtkSpd%", String.valueOf(pet.getPAtkSpd(true)));
