...
import static l2p.gameserver.model.quest.Quest.SOUND_FINISH;
...
public class _247_PossessorOfaPreciousSoul4 extends Quest implements ScriptFile {
    ...
    public String onTalk(NpcInstance npc, QuestState st) {
        ...
        int cond = st.getCond();
        if (npcId == CARADINE) {
            if (Config.ALT_GAME_NOOBLES_WITHOUT_QUESTS) {
                if (st.getPlayer().getLevel() >= 75 && !st.getPlayer().isNoble() && st.getQuestItemsCount(7679) == 1) {
                    st.giveItems(7694, 1);
                    st.addExpAndSp(93836, 0);
                    st.playSound(SOUND_FINISH);
                    Olympiad.addNoble(st.getPlayer());
                    ...
                    st.getPlayer().sendPacket(new SkillList(st.getPlayer()));
                    st.getPlayer().broadcastUserInfo(true);
                    return "Congratulations, you Noble!";
                }
            }
