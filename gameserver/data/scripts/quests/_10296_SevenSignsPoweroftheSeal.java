...
public class _10296_SevenSignsPoweroftheSeal extends Quest implements ScriptFile {
    ...
    public String onEvent(String event, QuestState st, NpcInstance npc) {
        ...
        if (event.equalsIgnoreCase("eris_q10296_3.htm")) {
            ...
            ThreadPoolManager.getInstance().schedule(new Teleport(player), 60500);
    ...
    private void addRew(Player player) {
        if (ScriptConfig.getBoolean("TrueSSAddonReward", false)) {
            ItemFunctions.addItem(player, ScriptConfig.getInt("SSAddonRewardItemId"), ScriptConfig.getInt("SSAddonRewardItemCount"), true);
            MultiSellHolder.getInstance().SeparateAndSend(ScriptConfig.getInt("MultiSellItemRewardId"), player, 0);
        }
    }
