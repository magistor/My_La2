...
public class PathfinderInstance extends NpcInstance {
    ...
    public void onBypassFeedback(Player player, String command) {
        ...
        if (command.startsWith("ExitSoloKama")) {
            ...
            if (!checkPlayerPa(player)) {
                showChatWindow(player, "instance/soloKamaloka/no_pa.htm");
                return;
            }
            int val = Integer.parseInt(command.substring(14));
			if (val >= 46 && val <= 56) // sanity check
			{
				Reflection newInstance = ReflectionUtils.simpleEnterInstancedZone(player, KamalokaNightmare.class, val);
				if (newInstance != null)
    ...
    private boolean checkPlayerPa(Player player) {
