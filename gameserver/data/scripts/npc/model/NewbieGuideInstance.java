            ...
            if (player.getClassId().getLevel() == 1) {
                ...
                if (player.getLevel() < 6) // FIXME: если получить 6 левел во время квеста то награду не дадут
                {
                    if (player.isQuestCompleted("_001_LettersOfLove") || player.isQuestCompleted("_002_WhatWomenWant") || player.isQuestCompleted("_004_LongLivethePaagrioLord") || player.isQuestCompleted("_005_MinersFavor") || player.isQuestCompleted("_166_DarkMass") || player.isQuestCompleted("_174_SupplyCheck")) {
                        ...
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q1-2.htm", val));
                        return;
                    } else {
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q1-1.htm", val).replace("%tonpc%", getQuestNpc(1, player)));
                        return;
                    }
                }
                if (player.getLevel() < 10) {
                    if (player.getVarB("p1q2")) {
                        ...
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q3-1.htm", val).replace("%tonpc%", getQuestNpc(3, player)));
                        return;
                    } else {
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q2-1.htm", val).replace("%tonpc%", getQuestNpc(2, player)));
                        return;
                    }
                }
                if (player.getLevel() < 15) {
                    if (player.getVarB("p1q3")) {
                        ...
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q4-1.htm", val).replace("%tonpc%", getQuestNpc(4, player)));
                        return;
                    } else {
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q3-1.htm", val).replace("%tonpc%", getQuestNpc(3, player)));
                        return;
                ...
                if (player.getLevel() < 18) {
                    if (player.getVarB("p1q4")) {
                        ...
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q4-2.htm", val));
                        return;
                    } else {
                        player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q4-1.htm", val).replace("%tonpc%", getQuestNpc(4, player)));
                        return;
                ...
                player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q-no.htm", val));
                return;
            } else {
                player.sendPacket(new HtmlMessage(player, this, "newbiehelper/q-no.htm", val));
                return;
