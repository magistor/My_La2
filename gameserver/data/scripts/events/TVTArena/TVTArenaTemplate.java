...
public abstract class TVTArenaTemplate extends Functions {
    ...
    private static final int[] KandinsDoorsToOpen = {17160033, 17160034, 17160038, 17160036, 17160035, 17160037, 17160040, 17160039};
    private static final int[] KoonDoorsToOpen = {17160029, 17160030, 17160028, 17160027, 17160026, 17160025, 17160032, 17160031};
    private static final int[] KuramDoorsToOpen = {17160023, 17160024, 17160018, 17160017, 17160020, 17160019, 17160022, 17160021};
    private static final int[] TarionDoorsToOpen = {17160011, 17160012, 17160010, 17160009, 17160016, 17160015, 17160014, 17160013};
    private static final int[] RioDoorsToOpen = {17160004, 17160003, 17160002, 17160001, 17160008, 17160007, 17160006, 17160005};
    ...
    public TVTArenaTemplate() {
        _status = 0;
        _CharacterFound = 0;

        _team1count = Config.EVENT_TVT_ARENA_TEAM_COUNT;
        _team2count = Config.EVENT_TVT_ARENA_TEAM_COUNT;
    ...
    public void template_register(Player player) {
        String html = HtmCache.getInstance().getHtml("scripts/events/TVTArena/" + _managerId + "-3.htm", player);
        ...
        for (Player plr : getPlayers(_team1list)) {
            playersBlueList = playersBlueList + plr.getName() + "<br1>";
        }
        for (Player plr : getPlayers(_team2list)) {
            playersRedList = playersRedList + plr.getName() + "<br1>";
        }
    ...
    public void template_announce() {
        Player creator = GameObjectsStorage.getAsPlayer(_creatorId.longValue());
    ...
    public void teleportTeamsToArena() {
        ...
        for (Player player : getPlayers(_team1live)) {
            ...
            player.teleToLocation((Location) _team1points.get(n), ReflectionManager.DEFAULT);
            localInteger1 = n;
            localInteger2 = n = n + 1;
        }
        ...
        for (Player player : getPlayers(_team2live)) {
            unRide(player);
            unSummonPet(player, true);
            player.teleToLocation((Location) _team2points.get(n), ReflectionManager.DEFAULT);
            localInteger1 = n;
            localInteger2 = n = n + 1;
        }
    }
        ...
        switch (nAlign) {
            case 0:
                for (Player plr : GameObjectsStorage.getAllPlayersForIterate()) {
                    if (plr.isOnline()) {
                        player.sendPacket(new ExShowScreenMessage(address, 3000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true));
                    }
                }
                break;
            case 1:
    ...
    public class ZoneListener
        ...
        public void onZoneLeave(Zone zone, Creature cha) {
            ...
            if ((_status >= 2) && (player != null) && (player.getTeam() != TeamType.NONE) && ((_team1list.contains(player.getStoredId())) || (_team2list.contains(player.getStoredId())))) {
                ...
                int x = (int) (cha.getX() + 50.0D * Math.sin(radian));
                int y = (int) (cha.getY() - 50.0D * Math.cos(radian));
                int z = cha.getZ();
                ThreadPoolManager.getInstance().schedule(new TVTArenaTemplate.TeleportTask(cha, new Location(x, y, z)), 3000L);
            }
        }
    }
}
