...
public class Viktorina extends Functions implements ScriptFile, IVoicedCommandHandler, OnPlayerEnterListener {
    ...
    public void loadQuestions() {
        File file = new File(Config.DATAPACK_ROOT + "/config/events/VictorinaQuestions.txt");
    ...
    public void announseViktorina(String text) {
        Say2 cs = new Say2(0, ChatType.TELL, "Викторина", text);
        for (Player player : GameObjectsStorage.getAllPlayersForIterate()) {
            if ("on".equals(player.getVar("viktorina"))) {
    ...
    public void checkPlayers() {
        Say2 cs = new Say2(0, ChatType.TELL, "Викторина", "чтобы отказаться от участия в викторине введите .voff , для справки введите .vhelp");
        for (Player player : GameObjectsStorage.getAllPlayersForIterate()) {
    ...
    /**
     * Функция продолжения таймера викторины, нужна при ручной остановке
     * викторины. Назначает старт викторины на след день
     */
    ...
    /**
     * Стоп вопроса: подсчитываем правильные ответы, и кто дал правильный ответ
     * быстрее всех. запускаем следующий вопрос.
     *
     * @author Sevil
     *
     */
    ...
    /**
     * Формируем окно справки. вызывается если игрок не разу не учавствовал в
     * викторине или командой .vhelp
     *
     * @param player
     */
    public void help(Player player) {
        ...
        if ("0".equals(ServerVariables.getString("viktorinaq", "0"))) {
            ServerVariables.set("viktorinaq", 0);
            vq = 0;
        } else {
        ...
        if ("0".equals(ServerVariables.getString("viktorinaa", "0"))) {
            ServerVariables.set("viktorinaa", 0);
            va = 0;
        } else {
        ...
        help.append(vstatus);
        help.append("Время начала викторины: ").append(VIKTORINA_START_TIME_HOUR).append(":").append(VIKTORINA_START_TIME_MIN).append("<br>");
        help.append("Длительность работы викторины ").append(VIKTORINA_WORK_TIME).append(" ч.<br>");
        help.append("Время в течении которого можно дать ответ: ").append(VIKTORINA_TIME_ANSER).append(" сек.<br>");
        help.append("Время между вопросами: ").append(VIKTORINA_TIME_ANSER + VIKTORINA_TIME_PAUSE).append(" сек.<br>");
        help.append("Вопросов уже было заданно: ").append(vq).append(".<br>");
        help.append("Верно ответили на : ").append(va).append(".<br>");
        help.append("Вы верно ответили на : ").append(schet).append(", в ").append(first).append(" вы были первым.<br>");
        help.append("<br>");
        help.append("<center>Команды викторины:<br></center>");
        help.append("<font color=\"LEVEL\">Ответ</font> - вводится в любой вид чата.<br>");
        help.append("<font color=\"LEVEL\">.von</font> - команда для включения витокрины<br>");
        help.append("<font color=\"LEVEL\">.voff</font> - команда для выключения витокрины<br>");
    ...
    /**
     * Выдаем приз на каторую укажет шанс + определяем выдавать приз для первого
     * или для остальных
     *
     * @param player
     * @param first
    ...
    private boolean giveItemByChance(Player player, boolean first) {
        ...
        for (RewardList items : _items) {
            ...
            if (first == items.getFirst() && chancesumm > chance) {
                ...
                addItem(player, productId, count, false);
                if (count > 1) {
    ...
    public void onPlayerEnter(Player player) {
        Say2 cs = new Say2(0, ChatType.CRITICAL_ANNOUNCE, "Викторина", "Активен ивент Викторина! Для участия наберите команду .von! Для справки .vhelp!");
        if (isStatus()) {
            player.sendPacket(cs);
        }
    }
