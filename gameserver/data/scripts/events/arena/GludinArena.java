...
public class GludinArena extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener {
    ...
    public String DialogAppend_17220015(Integer val) {
        if (val == 0) {
            ...
            if (player.isGM()) {
                return HtmCache.getInstance().getHtml("scripts/events/arena/17220015.htm", player) + HtmCache.getInstance().getHtml("scripts/events/arena/17220015-4.htm", player);
            }
            return HtmCache.getInstance().getHtml("scripts/events/arena/17220015.htm", player);
        }
        return "";
    }
