...
public class DionArena extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener {
    ...
    public String DialogAppend_20230001(Integer val) {
        if (val == 0) {
            ...
            if (player.isGM()) {
                return HtmCache.getInstance().getHtml("scripts/events/arena/20230001.htm", player) + HtmCache.getInstance().getHtml("scripts/events/arena/20230001-4.htm", player);
            }
            return HtmCache.getInstance().getHtml("scripts/events/arena/20230001.htm", player);
        }
        return "";
    }
