...
public class GiranArena extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener {
    ...
    public String DialogAppend_22220001(Integer val) {
        if (val == 0) {
            ...
            if (player.isGM()) {
                return HtmCache.getInstance().getHtml("scripts/events/arena/22220001.htm", player) + HtmCache.getInstance().getHtml("scripts/events/arena/22220001-4.htm", player);
            }
            return HtmCache.getInstance().getHtml("scripts/events/arena/22220001.htm", player);
        }
        return "";
    }
