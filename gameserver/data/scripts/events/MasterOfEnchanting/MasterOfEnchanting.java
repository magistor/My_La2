...
/**
 * Autor: Bonux Date: 30.08.09 Time: 17:49
...
public class MasterOfEnchanting extends Functions implements ScriptFile, OnPlayerEnterListener {
     ...
     * killer.getPlayer() != null && Math.abs(cha.getLevel() -
     * killer.getLevel()) < 10) { for(int[] drop : _herbdrop) if(Rnd.get(1000)
     * <= drop[1]) { L2ItemInstance item =
     * ItemTable.getInstance().createItem(drop[0],
     * killer.getPlayer().getObjectId(), 0, "Master of Enchanting");
     * ((L2NpcInstance) cha).dropItem(killer.getPlayer(), item);
     *
     * break; } for(int[] drop : _energydrop) if(Rnd.get(1000) <= drop[1]) {
     * L2ItemInstance item = ItemTable.getInstance().createItem(drop[0],
     * killer.getPlayer().getObjectId(), 0, "Master of Enchanting");
     * ((L2NpcInstance) cha).dropItem(killer.getPlayer(), item);
     *
     * break; } }
